...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = firebaseService.subscribeToEntries(
      user.uid,
      (entries) => {
  ...
  if (!state.onboarded)
    return (
      <Onboarding
        ...
        initialWeight={state.entries[state.entries.length - 1]?.weight}
      />
    );
    ...
    <div className="min-h-screen pb-24 md:pb-0 md:pl-20">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-line px-6 py-3 flex justify-between items-center z-50">
        ...
        <button
          ...
          className="w-14 h-14 bg-brand-500 rounded-full flex items-center justify-center text-white shadow-lg -mt-10 border-4 border-paper"
        >
          <Plus size={28} />
        </button>
        <MobileNavLink
      ...
      <main className="max-w-5xl mx-auto p-6 md:p-12">
        <AnimatePresence mode="wait">
          {activeTab === "dashboard" && (
            <Dashboard state={state} onLogClick={() => setIsLogging(true)} />
          )}
          {activeTab === "history" && (
            <HistoryView
              entries={state.entries}
              onDelete={deleteEntry}
              unit={state.goal?.unit || "lbs"}
            />
          )}
      ...
      <AnimatePresence>
        {isLogging && (
          <LogModal
            ...
            onSave={addEntry}
            unit={state.goal?.unit || "lbs"}
            lastWeight={state.entries[state.entries.length - 1]?.weight}
          />
        )}
      </AnimatePresence>
    </div>
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium">
            Precision weight tracking for focused progress.
          </p>
        </div>
...
function Dashboard({
  ...
  const trendData = useMemo(
    () =>
      analyticsService.getTrendData(
        state.entries,
        state.settings.smoothingWindow,
      ),
    [state.entries, state.settings.smoothingWindow],
  );
  const latest = trendData[trendData.length - 1];
  const milestones = useMemo(
    () => analyticsService.getMilestones(state.goal!),
    [state.goal],
  );
  const completed = useMemo(
    () => analyticsService.getCompletedMilestones(state.entries, state.goal!),
    [state.entries, state.goal],
  );
  const nextMilestone = milestones.find(
    (m) => !completed.find((c) => c.id === m.id),
  );
  ...
  const displayWeight = (w: number) =>
    state.settings.hideRawNumbers ? "—" : w.toFixed(1);
    ...
    >
        ...
        <div>
          ...
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight">
            ...
            <span className="text-brand-600">
              {latest?.trendWeight.toFixed(1)} {state.goal?.unit}
            </span>
          </h2>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <StatCard
          ...
          value={latest ? displayWeight(latest.weight) : "—"}
          unit={state.goal?.unit}
          subValue={latest ? `Trend: ${latest.trendWeight.toFixed(1)}` : ""}
          trend="neutral"
        />
        <StatCard
          ...
          unit={state.goal?.unit}
          subValue={`${Math.abs((latest?.trendWeight || 0) - (nextMilestone?.target || 0)).toFixed(1)} to go`}
          trend="down"
        />
        <StatCard
          label="Milestones"
          value={completed.length}
          unit={`of ${milestones.length}`}
                    ...
                    return (
                      <div className="bg-white p-3 rounded-xl shadow-xl border border-slate-100">
                        <p className="text-xs font-bold text-slate-400 mb-1">
                          {format(parseISO(payload[0].payload.date), "MMMM d")}
                        </p>
                        <p className="text-base font-bold text-brand-600">
                          Trend: {payload[0].value?.toFixed(1)}
                        </p>
                        {!state.settings.hideRawNumbers && (
                          <p className="text-xs text-slate-500">
                            Actual: {payload[1].value?.toFixed(1)}
                          </p>
                        )}
                      </div>
                    );
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isCompleted
                    ? "bg-brand-50 border-brand-100 text-brand-700 shadow-sm shadow-brand-50"
              ...
              >
                ...
                {isCompleted && (
                  <CheckCircle2 size={14} className="mx-auto mt-2" />
                )}
              </div>
            );
          })}
    ...
    <div className="bg-white p-6 rounded-2xl border border-line shadow-sm flex flex-col justify-between">
      <div>
        ...
        <div className="flex items-baseline gap-1">
          <h4 className="text-3xl font-bold text-ink tracking-tight">
            {value}
          </h4>
          {unit && (
        ...
        <div className="divide-y divide-line">
          {sorted.length === 0 ? (
            ...
            sorted.map((entry) => (
              <div
                key={entry.id}
                className="grid grid-cols-[1fr_auto_auto] md:grid-cols-[1fr_1fr_1fr_auto] items-center gap-4 py-4 px-6 hover:bg-slate-50 transition-colors group"
              >
                ...
                <div className="hidden md:flex flex-wrap gap-1.5">
                  {entry.tags.length > 0 ? (
                    entry.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] font-bold uppercase tracking-wider"
                      >
                        {tag}
                      </span>
                    ))
...
function InsightsView({ state }: { state: AppState }) {
  const rate = useMemo(
    () =>
      analyticsService.getRateOfChange(
        state.entries,
        state.settings.smoothingWindow,
      ),
    [state.entries, state.settings.smoothingWindow],
    ...
    >
        ...
        <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-xs text-amber-800 mt-1 leading-relaxed">
              Your weight on {format(parseISO(latestSpike.date), "MMM d")} was
              higher than your trend. This is normal water weight fluctuation.
              The trend line remains your primary indicator.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-line rounded-2xl p-8 shadow-sm">
            ...
            <div className="flex items-baseline gap-2">
              <span
                className={cn(
                  "text-5xl font-bold",
                  rate < 0 ? "text-brand-600" : "text-red-500",
                )}
              >
                {rate > 0 ? "+" : ""}
                {rate.toFixed(2)}
              ...
              <span className="text-slate-400 font-medium">
                {state.goal?.unit} / week
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-4 leading-relaxed">
              Based on your trailing 30-day trend. This velocity is used to
              calculate your projected goal dates.
            </p>
          </div>
                  ...
                  <div key={m.id} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-baseline">
                        ...
                        >
                          {m.target.toFixed(1)} {state.goal?.unit}
                        </span>
                        {isCompleted && (
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                            {format(parseISO(isCompleted.date), "MMM d, yyyy")}
                          </span>
                        )}
                      </div>
                    </div>
        ...
        <div className="bg-ink text-white rounded-2xl p-10 shadow-2xl flex flex-col justify-between">
          <div>
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-10">
              Goal Projection
            </h3>
            <div className="space-y-10">
              <div>
                ...
                <p className="text-4xl font-bold text-brand-400">
                  {predictions
                    ? format(predictions.likely, "MMMM d, yyyy")
                    : "Insufficient Data"}
                </p>
                <p className="text-xs text-slate-500 mt-2">
                  Calculated using your current 30-day average velocity.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 pt-10 border-t border-slate-800">
                <div>
                  ...
                  <p className="text-xl font-bold">
                    {predictions
                      ? format(predictions.optimistic, "MMMM d, yyyy")
                      : "—"}
                  </p>
                  <p className="text-[10px] text-slate-500 mt-1">
                    Assumes 20% increase in velocity.
                  </p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">
                  ...
                  <p className="text-xl font-bold">
                    {predictions
                      ? format(predictions.pessimistic, "MMMM d, yyyy")
                      : "—"}
                  </p>
                  <p className="text-[10px] text-slate-500 mt-1">
                    Assumes 20% decrease in velocity.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-10 p-4 bg-slate-900 rounded-xl border border-slate-800">
            <p className="text-[10px] text-slate-400 leading-relaxed italic">
              "Weight management is a marathon, not a sprint. Focus on the
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) =>
                  onUpdateGoal({ startWeight: parseFloat(e.target.value) })
                }
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) =>
                  onUpdateGoal({ targetWeight: parseFloat(e.target.value) })
                }
                className="w-full p-4 bg-slate-50 rounded-xl font-bold outline-none border border-transparent focus:border-brand-500 focus:bg-white transition-all"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                Milestone Size ({state.goal?.unit})
              </label>
              <input
                type="number"
                step="1"
                ...
                onChange={(e) =>
                  onUpdateGoal({ milestoneSize: parseFloat(e.target.value) })
                }
                className="w-full p-4 bg-slate-50 rounded-xl font-bold outline-none border border-transparent focus:border-brand-500 focus:bg-white transition-all"
              />
            </div>
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">
                    Days: {state.settings.smoothingWindow}
                  </p>
                </div>
                <input
                  type="range"
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() =>
                    onUpdateSettings({
                      hideRawNumbers: !state.settings.hideRawNumbers,
                    })
                  }
                  className={cn(
          ...
          <div className="bg-white p-8 rounded-2xl border border-line shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 gap-3">
              <button
                onClick={onExport}
                className="w-full p-4 bg-slate-900 text-white rounded-xl font-bold text-sm hover:bg-ink transition-all flex items-center justify-center gap-2"
              >
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="p-4 bg-white border border-line rounded-xl font-bold text-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-2 cursor-pointer">
                  ...
                  <input
                    ...
                    accept=".json"
                    onChange={(e) =>
                      e.target.files?.[0] && handleJson(e.target.files[0])
                    }
                    className="hidden"
                  />
                </label>
              ...
              <button
                onClick={onReset}
                className="w-full p-4 text-red-500 font-bold text-[10px] uppercase tracking-widest hover:bg-red-50 rounded-xl transition-all mt-4"
              >
                Reset All Data
              </button>
            </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      >
        ...
        <div className="space-y-8">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 mb-4 uppercase tracking-widest text-center">
              Any factors today?
            </p>
        ...
        <AnimatePresence mode="wait">
          {step === 1 ? (
            ...
            >
              ...
              <button
                onClick={() => setStep(2)}
                className="w-full py-5 bg-brand-500 text-white rounded-xl font-bold text-sm tracking-widest uppercase shadow-xl hover:bg-brand-600 transition-all active:scale-95"
              >
                Get Started
              </button>
            </motion.div>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full p-4 bg-slate-50 rounded-xl text-2xl font-bold outline-none border border-transparent focus:border-brand-500 focus:bg-white transition-all"
                    placeholder="0.0"
                  />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full p-4 bg-slate-50 rounded-xl font-bold outline-none border border-transparent focus:border-brand-500 focus:bg-white transition-all appearance-none"
                  >
              ...
              <button
                onClick={() =>
                  onComplete({
                    ...
                    milestoneSize: parseFloat(milestoneSize),
                  })
                }
